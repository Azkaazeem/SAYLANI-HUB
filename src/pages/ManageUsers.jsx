...
export default function ManageUsers() {
  ...
  const handleRoleChange = async (id, currentRole) => {
    ...
    if (!error) {
      Swal.fire({ icon: 'success', title: 'Role Updated!', text: 'User role has been changed successfully.', showConfirmButton: false, timer: 1500 });
      fetchUsers(); 
    } else {
  ...
  const handleDelete = async (id) => {
    ...
    if (!error) {
      Swal.fire({ icon: 'success', title: 'Deleted!', text: 'The user profile has been deleted.', showConfirmButton: false, timer: 1500 });
      fetchUsers();
    } else {
    ...
    <div className="space-y-6 relative">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white dark:bg-slate-800 p-4 rounded-xl shadow-sm transition-colors duration-300">
        <div className="flex space-x-2">
          ...
          <button onClick={() => setActiveTab('admin')} className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors flex items-center gap-2 ${activeTab === 'admin' ? 'bg-emerald-600 text-white' : 'bg-gray-100 dark:bg-slate-700 text-gray-600 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-slate-600'}`}>
        ...
        <div className="relative w-full md:w-64 text-gray-400 focus-within:text-gray-600 dark:focus-within:text-gray-200">
          ...
          <input type="text" placeholder="Search by name..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full bg-gray-100 dark:bg-slate-700 py-2 pl-10 pr-4 rounded-lg text-sm focus:outline-none dark:text-white transition-colors" />
        </div>
      </div>
                  ...
                  <tr key={user.id} className="hover:bg-gray-50 dark:hover:bg-slate-700/50 transition-colors">
                    ...
                    <td className="px-6 py-4 text-sm">
                      <span className={`px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider ${
                        (user.role || 'user') === 'admin' ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400' : 'bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400'
                      }`}>
                        {user.role || 'USER'}
                      </span>
                    </td>
                    ...
                    <td className="px-6 py-4 text-sm text-right flex justify-end space-x-2 items-center h-full pt-6">
                      <button onClick={() => handleRoleChange(user.id, user.role || 'user')} className={`p-2 rounded-lg transition-colors ${
                          (user.role || 'user') === 'admin' ? 'text-orange-500 hover:bg-orange-50 dark:hover:bg-orange-900/20' : 'text-emerald-500 hover:bg-emerald-50 dark:hover:bg-emerald-900/20'
      ...
      {isImageModalOpen && selectedImageUrl && (
        <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm transition-opacity animate-fade-in" onClick={closeImageModal}>
          <div className="relative bg-white dark:bg-slate-800 rounded-2xl overflow-hidden shadow-2xl max-w-4xl max-h-[90vh] flex flex-col" onClick={(e) => e.stopPropagation()}>
            <button onClick={closeImageModal} className="absolute top-4 right-4 bg-black/50 hover:bg-black/70 text-white p-2 rounded-full transition-colors z-10">
              <XCircle size={24} />
            </button>
            <div className="p-2 md:p-4 flex-1 overflow-auto flex items-center justify-center bg-gray-100 dark:bg-slate-900 min-w-[300px] min-h-[300px]">
               <img src={selectedImageUrl} alt="User Profile" className="max-w-full max-h-[85vh] object-contain rounded-lg shadow-md" />
            </div>
          </div>
        </div>
      )}
